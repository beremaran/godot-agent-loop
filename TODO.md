...
The current baseline is clean: build, lint, and all 487 tests pass. The items below cover improvements not currently caught by the test suite.

## High priority

- [ ] Strengthen filesystem sandboxing. `validatePath()` only rejects paths containing `..` and does not enforce project boundaries or resolve symlinks ([src/utils.ts:71](src/utils.ts:71)). Apply allowed-root checks consistently to editor launch, file operations, directory creation, project creation, and project configuration tools. Centralize path resolution using `realpath` and validate project-relative paths separately.
- [ ] Correct headless-process failure handling. `HeadlessOperationRunner` converts non-zero `execFile` exits into ordinary output ([src/headless-operation-runner.ts:29](src/headless-operation-runner.ts:29)), while the service only detects errors containing `Failed to` ([src/headless-operation-service.ts:21](src/headless-operation-service.ts:21)). Return exit status/signal information and treat every non-zero exit as a failure.
- [ ] Fix connection lifecycle races. `GameConnection.connect()` is asynchronous and not cancellable ([src/game-connection.ts:50](src/game-connection.ts:50)). Add connection generations/session IDs, cancel pending retries on disconnect, and ensure stale socket callbacks cannot mutate newer connection state.
