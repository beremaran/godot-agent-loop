...
export class ProjectToolHandlers {
  ...
  public async handleManageLayers(args: ToolArguments) {
    ...
    try {
      ...
      const sectionLayers = (): { type: string; layer: number; name: string }[] => {
        const section = /\[layer_names\]([\s\S]*?)(?=\n\[|$)/.exec(content);
        if (!section) return [];
        const layers: { type: string; layer: number; name: string }[] = [];
        const entry = /^(?:layer_names\/)?([\w_]+)\/layer_(\d+)\s*=\s*"([^"]*)"$/;
        for (const line of section[1].split('\n')) {
          const match = entry.exec(line.trim());
          if (match) layers.push({ type: match[1], layer: parseInt(match[2]), name: match[3] });
        }
        return layers;
      };
