...
import { importProjectResources, type E2EServer } from './helpers/harness.js';
 ...
 *
 * The whole suite shares one MCP server; each case works on a fresh
 * subproject, and the shared fixture stops the engine and reaps all Godot
...
describe('project settings and configuration tools through MCP', () => {
  ...
  it('read_project_settings parses the real project.godot and modify_project_settings is seen by the engine', async () => {
    ...
    const initial = await active.call('read_project_settings', { projectPath: fixture.project.projectPath });
    ...
    const raw = readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8');
  ...
  it('modify_project_settings creates a missing section and rejects invalid projects', async () => {
    ...
    const reread = await active.call('read_project_settings', { projectPath: fixture.project.projectPath });
    ...
    const missingProject = await active.call('modify_project_settings', {
      projectPath: join(fixture.server.root, 'not-a-project'),
      section: 'application',
      key: 'config/name',
      value: '"x"',
    });
  ...
  it('manage_autoloads list/add/remove is verified by the autoload actually loading in the engine', async () => {
    ...
    const empty = await active.call('manage_autoloads', { projectPath: fixture.project.projectPath, action: 'list' });
    ...
    const listed = await active.call('manage_autoloads', { projectPath: fixture.project.projectPath, action: 'list' });
    ...
    const removed = await active.call('manage_autoloads', {
      projectPath: fixture.project.projectPath, action: 'remove', name: 'E2EAutoload',
    });
    expect(removed.isError, removed.text).toBe(false);
    expect(payload((await active.call('manage_autoloads', { projectPath: fixture.project.projectPath, action: 'list' })).text))
    ...
    expect(readFileSync(join(fixture.project.projectPath, 'override.cfg'), 'utf8')).toContain('res://mcp_interaction_server.gd');
    expect(readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8')).not.toContain('mcp_interaction_server.gd');
  });

  it('manage_autoloads reports missing arguments and unknown actions', async () => {
    const active = fixture.server;
    const noName = await active.call('manage_autoloads', { projectPath: fixture.project.projectPath, action: 'add' });
    expect(noName.isError).toBe(true);
    expect(noName.text).toMatch(/name is required.*path is required/i);

    const bogus = await active.call('manage_autoloads', { projectPath: fixture.project.projectPath, action: 'sideload' });
  ...
  it('manage_input_map add/list/remove is verified through the live InputMap', async () => {
    ...
    const rawInputMap = readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8');
    expect(rawInputMap.match(/^e2e_jump=/gm)).toHaveLength(1);

    const listed = await active.call('manage_input_map', { projectPath: fixture.project.projectPath, action: 'list' });
    ...
    const removed = await active.call('manage_input_map', {
      projectPath: fixture.project.projectPath, action: 'remove', actionName: 'e2e_jump',
    });
    expect(removed.isError, removed.text).toBe(false);
    expect(payload((await active.call('manage_input_map', { projectPath: fixture.project.projectPath, action: 'list' })).text))
  ...
  it('manage_input_map defaults the deadzone and validates arguments', async () => {
    const active = fixture.server;
    const added = await active.call('manage_input_map', {
      projectPath: fixture.project.projectPath, action: 'add', actionName: 'e2e_default',
    });
    expect(added.isError, added.text).toBe(false);
    expect(readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8')).toContain('"deadzone": 0.5');

    const missing = await active.call('manage_input_map', { projectPath: fixture.project.projectPath, action: 'add' });
  ...
  it('manage_layers set/list is verified through the engine layer names', async () => {
    ...
    const reset = await active.call('manage_layers', {
      projectPath: fixture.project.projectPath, action: 'set', layerType: '2d_physics', layer: 3, name: 'hazards',
    });
    expect(reset.isError, reset.text).toBe(false);

    const listed = await active.call('manage_layers', { projectPath: fixture.project.projectPath, action: 'list' });
    ...
    expect(readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8'))
    ...
    const incomplete = await active.call('manage_layers', { projectPath: fixture.project.projectPath, action: 'set', layerType: '2d_physics' });
  ...
  it('set_main_scene changes which scene the engine actually boots', async () => {
    ...
    const set = await active.call('set_main_scene', { projectPath: fixture.project.projectPath, scenePath: 'alternate.tscn' });
    expect(set.isError, set.text).toBe(false);
    expect(set.text).toContain('res://alternate.tscn');
    expect(readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8'))
      .toContain('run/main_scene="res://alternate.tscn"');
    expect(readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8').match(/run\/main_scene=/g)).toHaveLength(1);
  ...
  it('manage_translations add/list/remove is verified through the engine locale list', async () => {
    ...
    const empty = await active.call('manage_translations', { projectPath: fixture.project.projectPath, action: 'list' });
    expect(payload(empty.text)).toEqual({ translations: [] });

    // A real .po compiles into the engine's TranslationServer at boot.
    writeFileSync(join(fixture.project.projectPath, 'strings.fr.po'), [
      'msgid ""', 'msgstr ""',
      '"Language: fr\\n"',
    ...
    const added = await active.call('manage_translations', {
      projectPath: fixture.project.projectPath, action: 'add', translationPath: 'strings.fr.po',
    });
    expect(added.isError, added.text).toBe(false);
    expect(added.text).toContain('res://strings.fr.po');

    const listed = await active.call('manage_translations', { projectPath: fixture.project.projectPath, action: 'list' });
    ...
    const removed = await active.call('manage_translations', {
      projectPath: fixture.project.projectPath, action: 'remove', translationPath: 'strings.fr.po',
    });
    expect(removed.isError, removed.text).toBe(false);
    expect(payload((await active.call('manage_translations', { projectPath: fixture.project.projectPath, action: 'list' })).text))
  ...
  it('manage_plugins list/enable/disable round-trips through project.godot', async () => {
    ...
    const bare = await active.call('manage_plugins', { projectPath: fixture.project.projectPath, action: 'list' });
    expect(payload(bare.text)).toEqual({ enabled: [], available: [] });

    mkdirSync(join(fixture.project.projectPath, 'addons', 'sample_plugin'), { recursive: true });
    writeFileSync(join(fixture.project.projectPath, 'addons', 'sample_plugin', 'plugin.cfg'), [
      '[plugin]', '', 'name="Sample Plugin"', 'description=""', 'author=""',
      'version="1.0"', 'script="plugin.gd"', '',
    ].join('\n'));

    const discovered = await active.call('manage_plugins', { projectPath: fixture.project.projectPath, action: 'list' });
    expect(payload(discovered.text)).toEqual({ enabled: [], available: ['sample_plugin'] });

    const enabled = await active.call('manage_plugins', {
      projectPath: fixture.project.projectPath, action: 'enable', pluginName: 'sample_plugin',
    });
    expect(enabled.isError, enabled.text).toBe(false);
    expect(payload((await active.call('manage_plugins', { projectPath: fixture.project.projectPath, action: 'list' })).text))
      .toEqual({ enabled: ['sample_plugin'], available: ['sample_plugin'] });

    const disabled = await active.call('manage_plugins', {
      projectPath: fixture.project.projectPath, action: 'disable', pluginName: 'sample_plugin',
    });
    ...
    expect(readFileSync(join(fixture.project.projectPath, 'project.godot'), 'utf8').match(/^enabled=PackedStringArray\(/gm)).toHaveLength(1);

    const unknown = await active.call('manage_plugins', { projectPath: fixture.project.projectPath, action: 'purge' });
  ...
  it('manage_export_presets list/add/remove round-trips through export_presets.cfg', async () => {
    ...
    const empty = await active.call('manage_export_presets', { projectPath: fixture.project.projectPath, action: 'list' });
    expect(payload(empty.text)).toEqual({ presets: [] });

    const added = await active.call('manage_export_presets', {
      projectPath: fixture.project.projectPath, action: 'add', name: 'LinuxE2E', platform: 'Linux/X11', runnable: true,
    });
    expect(added.isError, added.text).toBe(false);
    expect(existsSync(join(fixture.project.projectPath, 'export_presets.cfg'))).toBe(true);

    const listed = await active.call('manage_export_presets', { projectPath: fixture.project.projectPath, action: 'list' });
    ...
    const removed = await active.call('manage_export_presets', {
      projectPath: fixture.project.projectPath, action: 'remove', name: 'LinuxE2E',
    });
    expect(removed.isError, removed.text).toBe(false);
    const after = await active.call('manage_export_presets', { projectPath: fixture.project.projectPath, action: 'list' });
...
describe('project file and script tools through MCP', () => {
  it('write_file/read_file/rename_file/delete_file/create_directory round-trip on the real filesystem', async () => {
    ...
    const written = await active.call('write_file', {
      projectPath: fixture.project.projectPath, filePath: 'data/notes.txt', content: 'héllo → wörld\nsecond line\n',
    });
    expect(written.isError, written.text).toBe(false);
    // write_file creates intermediate directories.
    expect(existsSync(join(fixture.project.projectPath, 'data', 'notes.txt'))).toBe(true);
    ...
    const read = await active.call('read_file', { projectPath: fixture.project.projectPath, filePath: 'data/notes.txt' });
    expect(read.isError, read.text).toBe(false);
    expect(read.text).toBe('héllo → wörld\nsecond line\n');

    const dir = await active.call('create_directory', { projectPath: fixture.project.projectPath, directoryPath: 'data/nested/deep' });
    expect(dir.isError, dir.text).toBe(false);
    expect(existsSync(join(fixture.project.projectPath, 'data', 'nested', 'deep'))).toBe(true);

    const renamed = await active.call('rename_file', {
      projectPath: fixture.project.projectPath, filePath: 'data/notes.txt', newPath: 'data/nested/deep/moved.txt',
    });
    expect(renamed.isError, renamed.text).toBe(false);
    expect(existsSync(join(fixture.project.projectPath, 'data', 'notes.txt'))).toBe(false);
    const moved = await active.call('read_file', { projectPath: fixture.project.projectPath, filePath: 'data/nested/deep/moved.txt' });
    expect(moved.text).toBe('héllo → wörld\nsecond line\n');

    const deleted = await active.call('delete_file', {
      projectPath: fixture.project.projectPath, filePath: 'data/nested/deep/moved.txt',
    });
    expect(deleted.isError, deleted.text).toBe(false);
    expect(existsSync(join(fixture.project.projectPath, 'data', 'nested', 'deep', 'moved.txt'))).toBe(false);

    const gone = await active.call('read_file', { projectPath: fixture.project.projectPath, filePath: 'data/nested/deep/moved.txt' });
  ...
  it('file tools reject traversal and absolute escapes without touching the filesystem', async () => {
    const active = fixture.server;
    const secret = join(fixture.server.root, 'outside.txt');
    writeFileSync(secret, 'do-not-touch');

    for (const filePath of ['../outside.txt', 'data/../../outside.txt', '/etc/passwd']) {
      const read = await active.call('read_file', { projectPath: fixture.project.projectPath, filePath });
      expect(read.isError, `read_file must reject ${filePath}`).toBe(true);
      expect(read.text).toMatch(/invalid path|outside the project/i);

      const write = await active.call('write_file', { projectPath: fixture.project.projectPath, filePath, content: 'pwned' });
      expect(write.isError, `write_file must reject ${filePath}`).toBe(true);

      const remove = await active.call('delete_file', { projectPath: fixture.project.projectPath, filePath });
    ...
    const missingFile = await active.call('delete_file', { projectPath: fixture.project.projectPath, filePath: 'nope.txt' });
    ...
    const missingRename = await active.call('rename_file', {
      projectPath: fixture.project.projectPath, filePath: 'nope.txt', newPath: 'other.txt',
    });
  ...
  it('write_file reports a read-only target without corrupting it', async () => {
    const active = fixture.server;
    const target = join(fixture.project.projectPath, 'locked.txt');
    writeFileSync(target, 'original');
    chmodSync(target, 0o444);
    try {
      const result = await active.call('write_file', {
        projectPath: fixture.project.projectPath, filePath: 'locked.txt', content: 'replacement',
      });
  ...
  it('list_project_files filters by extension and subdirectory', async () => {
    const active = fixture.server;
    await active.call('write_file', { projectPath: fixture.project.projectPath, filePath: 'src/a.gd', content: 'extends Node\n' });
    await active.call('write_file', { projectPath: fixture.project.projectPath, filePath: 'src/b.txt', content: 'text\n' });
    await active.call('write_file', { projectPath: fixture.project.projectPath, filePath: 'src/deep/c.gd', content: 'extends Node\n' });

    const all = await active.call('list_project_files', { projectPath: fixture.project.projectPath });
    ...
    const scripts = await active.call('list_project_files', { projectPath: fixture.project.projectPath, extensions: ['.gd'] });
    ...
    const scoped = await active.call('list_project_files', {
      projectPath: fixture.project.projectPath, subdirectory: 'src', extensions: ['.gd'],
    });
    const scopedFiles = (payload(scoped.text) as { files: string[] }).files;
    expect(scopedFiles.sort()).toEqual(['src/a.gd', 'src/deep/c.gd']);

    const missing = await active.call('list_project_files', { projectPath: fixture.project.projectPath, subdirectory: 'absent' });
    expect(missing.isError).toBe(true);
  ...
  it('list_project_files paginates a large project deterministically', async () => {
    const active = fixture.server;
    const largeDir = join(fixture.project.projectPath, 'large');
    mkdirSync(largeDir);
    ...
    const first = await active.call('list_project_files', {
      projectPath: fixture.project.projectPath, subdirectory: 'large', limit: 1000,
    });
    ...
    const second = await active.call('list_project_files', {
      projectPath: fixture.project.projectPath, subdirectory: 'large', limit: 1000, cursor: firstPage.nextCursor,
    });
    ...
    const invalidLimit = await active.call('list_project_files', {
      projectPath: fixture.project.projectPath, limit: 1001,
    });
  ...
  it('create_script builds source from options and Godot parses the result', async () => {
    ...
    const source = await active.call('read_file', { projectPath: fixture.project.projectPath, filePath: 'scripts/player.gd' });
    ...
    const validated = await active.call('validate_script', {
      projectPath: fixture.project.projectPath, scriptPath: 'scripts/player.gd',
    });
    ...
    const explicit = await active.call('create_script', {
      projectPath: fixture.project.projectPath, scriptPath: 'scripts/verbatim.gd', source: 'extends Area2D\n',
    });
    expect(explicit.isError, explicit.text).toBe(false);
    expect((await active.call('read_file', { projectPath: fixture.project.projectPath, filePath: 'scripts/verbatim.gd' })).text)
      .toBe('extends Area2D\n');

    const defaulted = await active.call('create_script', { projectPath: fixture.project.projectPath, scriptPath: 'scripts/plain.gd' });
    expect(defaulted.isError, defaulted.text).toBe(false);
    expect((await active.call('read_file', { projectPath: fixture.project.projectPath, filePath: 'scripts/plain.gd' })).text)
  ...
  it('validate_script reports real Godot parse errors and rejects non-GDScript', async () => {
    ...
    const invalid = await active.call('validate_script', {
      projectPath: fixture.project.projectPath, scriptPath: 'scripts/broken.gd',
    });
    ...
    const wrongType = await active.call('validate_script', {
      projectPath: fixture.project.projectPath, scriptPath: 'main.tscn',
    });
    expect(wrongType.isError).toBe(true);
    expect(wrongType.text).toMatch(/only checks GDScript/i);

    const absent = await active.call('validate_script', {
      projectPath: fixture.project.projectPath, scriptPath: 'scripts/absent.gd',
    });
  ...
  it('validate_script reports a fresh structured compile for a rewritten source through MCP', async () => {
    ...
    const valid = await active.call('validate_script', { projectPath: fixture.project.projectPath, scriptPath });
    ...
    const invalid = await active.call('validate_script', { projectPath: fixture.project.projectPath, scriptPath });
    expect(invalid.isError, invalid.text).toBe(false);
  ...
  it('validate_scripts covers explicit, changed, all, and invalid scopes against real Godot', async () => {
    const active = fixture.server;
    await active.call('create_script', { projectPath: fixture.project.projectPath, scriptPath: 'scripts/ok.gd', extends: 'Node' });
    await active.call('write_file', {
      projectPath: fixture.project.projectPath, filePath: 'scripts/bad.gd', content: 'extends Node\n\nfunc broken(:\n',
    });

    const explicit = await active.call('validate_scripts', {
      projectPath: fixture.project.projectPath, scriptPaths: ['scripts/ok.gd', 'scripts/bad.gd', 'scripts/ghost.gd', 'main.tscn'],
    });
    expect(explicit.isError, explicit.text).toBe(false);
    ...
    execFileSync('git', ['init', '--quiet'], { cwd: fixture.project.projectPath });
    const changed = await active.call('validate_scripts', { projectPath: fixture.project.projectPath, scope: 'changed' });
    ...
    const all = await active.call('validate_scripts', { projectPath: fixture.project.projectPath, scope: 'all' });
    expect(all.isError, all.text).toBe(false);
    ...
    const invalidScope = await active.call('validate_scripts', {
      projectPath: fixture.project.projectPath, scope: 'sideways',
    });
  ...
  it('manage_shader create/read produces a shader the engine compiles', async () => {
    ...
    const created = await active.call('manage_shader', {
      projectPath: fixture.project.projectPath, shaderPath: 'shaders/tint.gdshader', action: 'create', shaderType: 'canvas_item',
    });
    expect(created.isError, created.text).toBe(false);

    const read = await active.call('manage_shader', {
      projectPath: fixture.project.projectPath, shaderPath: 'shaders/tint.gdshader', action: 'read',
    });
    expect(read.isError, read.text).toBe(false);
    ...
    const missing = await active.call('manage_shader', {
      projectPath: fixture.project.projectPath, shaderPath: 'shaders/absent.gdshader', action: 'read',
    });
    expect(missing.isError).toBe(true);
