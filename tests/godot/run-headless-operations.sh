...
# Direct engine diagnostics for the operations in src/scripts/godot_operations.gd.
#
# Coverage ownership (see docs/coverage/evidence-ownership.md): this script owns
# the raw engine seam — operation-script argument parsing and the exit-status
# contract the MCP runner relies on, engine diagnostics, and engine-specific
# invariants — plus one representative positive per operation family. The MCP
# E2E suite (tests/e2e/headless-tools.test.ts) owns transport, tool dispatch,
# structured responses, persistence through the public boundary, and cleanup.
# Do not add extra positive variants here: the E2E suite is the authoritative
# owner of persistence and parameter-family breadth.
#
...
echo
echo "Scene operations (one representative positive per operation)"
...
echo
echo "Scene structure operations (duplicate and move stay covered by the regression suite below)"
...
run_op remove_node '{"scene_path":"scenes/main.tscn","node_path":"Hero"}'
...
echo
echo "Failure handling: operation-script parsing and exit-status contract"
...
echo
echo "Persistent authoring session (windowless)"
...
GODOT_MCP_FIXED_FPS=60 GODOT_MCP_RUNTIME_PORT="$SESSION_PORT" GODOT_MCP_RUNTIME_SECRET="$SESSION_SECRET" \
  "$GODOT" --headless --audio-driver Dummy --fixed-fps 60 --max-fps 60 --time-scale 1 \
